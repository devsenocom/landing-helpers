...
Релиз создаётся на **любой** пуш в `main`, кроме правок `*.md`, `.github/**`,
`.vscode/**`, `.gitignore` и `.gitattributes`. Всё остальное в репозитории —
контент, который раздаётся с CDN, поэтому триггер сделан списком исключений, а
не списком путей: новая папка проекта с фавиконкой (`lyssa/favicon.webp`)
выкатывается сама, без правки workflow.

Дальше: сборка → коммит пересобранных файлов → новый тег (patch +1) →
GitHub Release → purge кеша jsDelivr.

Пуржатся оба хелпера плюс все файлы, затронутые этим пушем. Это важно для замены
существующей картинки: без purge перезалитая фавиконка осталась бы в кеше CDN в
старом виде.
